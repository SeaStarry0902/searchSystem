    ...
    <div class="flex flex-col">
        ...
        <div v-if="options.length === 0" class="flex relative">
            <input v-model="model" :type="show ? 'text' : 'password'"
                :class="customWidth ? customWidth : ' w-[clamp(0px,40vmin,9999px)]'"
                class=" h-[clamp(0px,6vmin,9999px)] bg-white border rounded-[clamp(0px,1vmin,40px)] pl-1 md:pl-2 2xl:pl-4 pr-7 md:pr-10 2xl:pr-14 text-xs md:text-xl 2xl:text-2xl" />
        ...
        <div v-if="options.length !== 0" class="flex relative ">
            <select v-model="model" :class="customWidth ? customWidth : ' w-[clamp(0px,40vmin,9999px)]'"
                class="cursor-pointer h-[clamp(0px,6vmin,9999px)] bg-white border rounded-lg pl-1 md:pl-2 2xl:pl-4 pr-7 md:pr-10 2xl:pr-14 text-xs md:text-xl 2xl:text-2xl">
            >
