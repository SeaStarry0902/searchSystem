...
<template>
    ...
    <div v-if="showTable" class="mt-10 flex flex-col items-center overflow-auto w-full gap-5">
        <div
            class="flex gap-2 items-end justify-end w-[calc(60px+100px+180px+40px+320px+100px+80px+40px+200px+100px+40px+100px+200px)]">
            ...
            <button class="bg-[#6ffc92] w-30 h-10 border rounded cursor-pointer hover:bg-[#23f157] hover:font-bold"
                @click="downloadExcel()">匯入</button>
        </div>
        <div
            class=" inline-grid h-auto grid-cols-[60px_100px_180px_40px_320px_100px_80px_40px_200px_100px_40px_100px_200px]  ">
            ...
            <template v-for="(course, idx) in courses" :key="course.id">
                <div class="h-16 bg-[#EEEEEE] flex flex-col border border-[#146d2b] items-center justify-center">
                    {{ idx + 1 }}
                </div>
                <div class="h-16 bg-[#EEEEEE] flex flex-col border border-[#146d2b] items-center justify-center">
                    {{ course.semester }}
                ...
                <div class="h-16 bg-[#EEEEEE] flex flex-col border border-[#146d2b] items-center justify-center">
                    {{ course.start_section === course.end_section ? course.start_section : '第' + course.start_section +
                        '~' +
                        course.end_section }}節
                </div>
                <div class="h-16 bg-[#EEEEEE] gap-2 flex border items-center justify-center">
                    <span v-if="!course.editing"
                        class="bg-[#1878d8] px-4 py-2 border rounded cursor-pointer hover:bg-[#0061c2]"
                        @click="startEdit(course)">
                        編輯
                    </span>
                    <span v-if="course.editing"
                        class="bg-green-600 px-4 py-2 border rounded cursor-pointer hover:bg-green-700"
                        @click="saveCourse(course)">
                        儲存
                    </span>
                    <span v-if="course.editing"
                        class="bg-gray-500 px-4 py-2 border rounded cursor-pointer hover:bg-gray-600"
                        @click="cancelEdit(course)">
                        取消
                    </span>
                    <span v-if="!course.editing"
                        class="bg-amber-700 px-4 py-2 border rounded cursor-pointer hover:bg-amber-800"
                        @click="deleteCheck = true; selectedUserId = course.id">
                        刪除
                    </span>
                </div>
            </template>
